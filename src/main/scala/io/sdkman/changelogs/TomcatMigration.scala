...
class TomcatMigration {
  ...
  def migration025(implicit db: MongoDatabase): Document = {
    List(
      ...
      .validate()
      .insert()
    setCandidateDefault("tomcat", "11.0.14")
  }

  @ChangeSet(
    order = "026",
    id = "026-update_tomcat_versions",
    author = "stefanpenndorf"
  )
  def migration026(implicit db: MongoDatabase): Document = {
    List(
      "9"  -> "9.0.115",
      "10" -> "10.1.52",
      "11" -> "11.0.18"
