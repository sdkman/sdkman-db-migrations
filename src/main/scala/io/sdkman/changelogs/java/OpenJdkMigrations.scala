...
class OpenJdkMigrations {
  ...
  @ChangeSet(
    order = "035",
    id = "035-add_openjdk_java_15-ea-16",
    author = "eddumelendez"
  )
  def migrate035(implicit db: MongoDatabase): Unit = {
    List(
      Version(
        "java",
        "15.ea.16-open",
        "https://download.java.net/java/early_access/jdk15/16/GPL/openjdk-15-ea+16_linux-x64_bin.tar.gz",
        Linux64,
        Some(OpenJDK)
      ),
      Version(
        "java",
        "15.ea.16-open",
        "https://download.java.net/java/early_access/jdk15/16/GPL/openjdk-15-ea+16_osx-x64_bin.tar.gz",
        MacOSX,
        Some(OpenJDK)
      ),
      Version(
        "java",
        "15.ea.16-open",
        "https://download.java.net/java/early_access/jdk15/16/GPL/openjdk-15-ea+16_windows-x64_bin.zip",
        Windows,
        Some(OpenJDK)
      )
    ).validate()
