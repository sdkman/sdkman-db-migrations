...
class OpenJdkMigrations {
  ...
  @ChangeSet(
    order = "080",
    id = "080-add_openjdk_java_16-ea-19",
    author = "mnedic"
  )
  def migrate080(implicit db: MongoDatabase): Unit =
    Map(
      LinuxARM64 -> "openjdk-16-ea+19_linux-aarch64_bin.tar.gz",
      Linux64    -> "openjdk-16-ea+19_linux-x64_bin.tar.gz",
      MacOSX     -> "openjdk-16-ea+19_osx-x64_bin.tar.gz",
      Windows    -> "openjdk-16-ea+19_windows-x64_bin.zip"
    ).map {
        case (platform, binary) =>
          Version(
            "java",
            "16.ea.19-open",
            s"https://download.java.net/java/early_access/jdk16/19/GPL/$binary",
            platform,
            Some(OpenJDK)
          )
      }
