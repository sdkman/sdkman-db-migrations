...
class OpenJdkMigrations {
  ...
  @ChangeSet(
    order = "089",
    id = "089-add_openjdk_java_16-loom-7",
    author = "soberich"
  )
  def migrate089(implicit db: MongoDatabase): Unit =
    Map(
      Linux64 -> "openjdk-16-loom+7-285_linux-x64_bin.tar.gz",
      MacOSX  -> "openjdk-16-loom+7-285_osx-x64_bin.tar.gz",
      Windows -> "openjdk-16-loom+7-285_windows-x64_bin.zip"
    ).map {
        case (platform, binary) =>
          Version(
            "java",
            "16.ea.7.lm-open",
            s"https://download.java.net/java/early_access/loom/7/$binary",
            platform,
            Some(OpenJDK)
          )
      }
      ...
      .insert()
      .foreach { version =>
        removeVersion("java", "16.ea.6.lm-open", version.platform)
      }

  @ChangeSet(
    order = "090",
    id = "090-add_openjdk_java_16-ea+24",
    author = "eddumelendez"
  )
  def migrate090(implicit db: MongoDatabase): Unit =
    Map(
      LinuxARM64 -> "openjdk-16-ea+24_linux-aarch64_bin.tar.gz",
      Linux64    -> "openjdk-16-ea+24_linux-x64_bin.tar.gz",
      MacOSX     -> "openjdk-16-ea+24_osx-x64_bin.tar.gz",
      Windows    -> "openjdk-16-ea+24_windows-x64_bin.zip"
    ).map {
        case (platform, binary) =>
          Version(
            "java",
            "16.ea.24-open",
            s"https://download.java.net/java/early_access/jdk16/24/GPL/$binary",
            platform,
            Some(OpenJDK)
          )
      }
      ...
      .insert()
      .foreach { version =>
        removeVersion("java", "16.ea.23-open", version.platform)
      }
