...
class OpenJdkMigrations {
  ...
  @ChangeSet(
    order = "043",
    id = "043-add_openjdk_java_15-ea-22",
    author = "eddumelendez"
  )
  def migrate043(implicit db: MongoDatabase): Unit = {
    List(
      Version(
        "java",
        "15.ea.22-open",
        "https://download.java.net/java/early_access/jdk15/22/GPL/openjdk-15-ea+22_linux-x64_bin.tar.gz",
        Linux64,
        Some(OpenJDK)
      ),
      Version(
        "java",
        "15.ea.22-open",
        "https://download.java.net/java/early_access/jdk15/22/GPL/openjdk-15-ea+22_osx-x64_bin.tar.gz",
        MacOSX,
        Some(OpenJDK)
      ),
      Version(
        "java",
        "15.ea.22-open",
        "https://download.java.net/java/early_access/jdk15/22/GPL/openjdk-15-ea+22_windows-x64_bin.zip",
        Windows,
        Some(OpenJDK)
      )
    ).validate()
      .insert()
    Seq(Linux64, MacOSX, Windows).foreach(
      removeVersion("java", "15.ea.21-open", _)
    )
  }

  @ChangeSet(
    order = "044",
    id = "044-add_openjdk_java_15-ea-23",
    author = "eddumelendez"
  )
  def migrate044(implicit db: MongoDatabase): Unit = {
    List(
      Version(
        "java",
        "15.ea.23-open",
        "https://download.java.net/java/early_access/jdk15/23/GPL/openjdk-15-ea+23_linux-x64_bin.tar.gz",
        Linux64,
        Some(OpenJDK)
      ),
      Version(
        "java",
        "15.ea.23-open",
        "https://download.java.net/java/early_access/jdk15/23/GPL/openjdk-15-ea+23_osx-x64_bin.tar.gz",
        MacOSX,
        Some(OpenJDK)
      ),
      Version(
        "java",
        "15.ea.23-open",
        "https://download.java.net/java/early_access/jdk15/23/GPL/openjdk-15-ea+23_windows-x64_bin.zip",
        Windows,
        Some(OpenJDK)
      )
    ).validate()
      .insert()
    Seq(Linux64, MacOSX, Windows).foreach(
      removeVersion("java", "15.ea.22-open", _)
    )
  }

  @ChangeSet(
    order = "045",
    id = "045-add_openjdk_java_15-ea-24",
    author = "eddumelendez"
  )
  def migrate045(implicit db: MongoDatabase): Unit = {
    List(
      Version(
        "java",
        "15.ea.24-open",
        "https://download.java.net/java/early_access/jdk15/24/GPL/openjdk-15-ea+24_linux-x64_bin.tar.gz",
        Linux64,
        Some(OpenJDK)
      ),
      Version(
        "java",
        "15.ea.24-open",
        "https://download.java.net/java/early_access/jdk15/24/GPL/openjdk-15-ea+24_osx-x64_bin.tar.gz",
        MacOSX,
        Some(OpenJDK)
      ),
      Version(
        "java",
        "15.ea.24-open",
        "https://download.java.net/java/early_access/jdk15/24/GPL/openjdk-15-ea+24_windows-x64_bin.zip",
        Windows,
        Some(OpenJDK)
      )
    ).validate()
      .insert()
    Seq(Linux64, MacOSX, Windows).foreach(
      removeVersion("java", "15.ea.23-open", _)
    )
  }

  @ChangeSet(
    order = "046",
    id = "046-add_openjdk_java_15-ea-25",
    author = "eddumelendez"
  )
  def migrate046(implicit db: MongoDatabase): Unit = {
    List(
      Version(
        "java",
        "15.ea.25-open",
        "https://download.java.net/java/early_access/jdk15/25/GPL/openjdk-15-ea+25_linux-x64_bin.tar.gz",
        Linux64,
        Some(OpenJDK)
      ),
      Version(
        "java",
        "15.ea.25-open",
        "https://download.java.net/java/early_access/jdk15/25/GPL/openjdk-15-ea+25_osx-x64_bin.tar.gz",
        MacOSX,
        Some(OpenJDK)
      ),
      Version(
      ...
      .insert()
    Seq(Linux64, MacOSX, Windows).foreach(
      removeVersion("java", "15.ea.24-open", _)
    )
  }

  @ChangeSet(
    order = "047",
    id = "047-add_openjdk_java_15-ea-26",
    author = "eddumelendez"
  )
  def migrate047(implicit db: MongoDatabase): Unit = {
    List(
      ...
      .insert()
    Seq(Linux64, MacOSX, Windows).foreach(
      removeVersion("java", "15.ea.25-open", _)
    )
  }

  @ChangeSet(
    order = "048",
    id = "048-add_openjdk_java_15-ea-27",
    author = "eddumelendez"
  )
  def migrate048(implicit db: MongoDatabase): Unit = {
    List(
      Version(
        "java",
        "15.ea.27-open",
        "https://download.java.net/java/early_access/jdk15/27/GPL/openjdk-15-ea+27_linux-x64_bin.tar.gz",
        Linux64,
        Some(OpenJDK)
      ),
      Version(
        "java",
        "15.ea.27-open",
        "https://download.java.net/java/early_access/jdk15/27/GPL/openjdk-15-ea+27_osx-x64_bin.tar.gz",
        MacOSX,
        Some(OpenJDK)
      ),
      Version(
        "java",
        "15.ea.27-open",
        "https://download.java.net/java/early_access/jdk15/27/GPL/openjdk-15-ea+27_windows-x64_bin.zip",
        Windows,
        Some(OpenJDK)
      )
    ).validate()
      .insert()
    Seq(Linux64, MacOSX, Windows).foreach(
      removeVersion("java", "15.ea.26-open", _)
    )
  }

  @ChangeSet(
    order = "049",
    id = "049-add_openjdk_java_16-ea-1",
    author = "eddumelendez"
  )
  def migrate049(implicit db: MongoDatabase): Unit = {
    List(
      Version(
        "java",
        "16.ea.1-open",
        "https://download.java.net/java/early_access/jdk16/1/GPL/openjdk-16-ea+1_linux-x64_bin.tar.gz",
        Linux64,
        Some(OpenJDK)
      ),
      Version(
        "java",
        "16.ea.1-open",
        "https://download.java.net/java/early_access/jdk16/1/GPL/openjdk-16-ea+1_osx-x64_bin.tar.gz",
        MacOSX,
        Some(OpenJDK)
      ),
      Version(
        "java",
        "16.ea.1-open",
        "https://download.java.net/java/early_access/jdk16/1/GPL/openjdk-16-ea+1_windows-x64_bin.zip",
        Windows,
        Some(OpenJDK)
      )
    ).validate()
  ...
  @ChangeSet(
    order = "051",
    id = "051-add_openjdk_java_15-ea-28",
    author = "eddumelendez"
  )
  def migrate051(implicit db: MongoDatabase): Unit = {
    List(
      Version(
        "java",
        "15.ea.28-open",
        "https://download.java.net/java/early_access/jdk15/28/GPL/openjdk-15-ea+28_linux-x64_bin.tar.gz",
        Linux64,
        Some(OpenJDK)
      ),
      Version(
        "java",
        "15.ea.28-open",
        "https://download.java.net/java/early_access/jdk15/28/GPL/openjdk-15-ea+28_osx-x64_bin.tar.gz",
        MacOSX,
        Some(OpenJDK)
      ),
      Version(
        "java",
        "15.ea.28-open",
        "https://download.java.net/java/early_access/jdk15/28/GPL/openjdk-15-ea+28_windows-x64_bin.zip",
        Windows,
        Some(OpenJDK)
      )
    ).validate()
      .insert()
    Seq(Linux64, MacOSX, Windows).foreach(
      removeVersion("java", "15.ea.27-open", _)
    )
  }

  @ChangeSet(
    order = "052",
    id = "052-add_openjdk_java_15-ea-29",
    author = "eddumelendez"
  )
  def migrate052(implicit db: MongoDatabase): Unit = {
    List(
      Version(
        "java",
        "15.ea.29-open",
        "https://download.java.net/java/early_access/jdk15/29/GPL/openjdk-15-ea+29_linux-x64_bin.tar.gz",
        Linux64,
        Some(OpenJDK)
      ),
      Version(
        "java",
        "15.ea.29-open",
        "https://download.java.net/java/early_access/jdk15/29/GPL/openjdk-15-ea+29_osx-x64_bin.tar.gz",
        MacOSX,
        Some(OpenJDK)
      ),
      Version(
        "java",
        "15.ea.29-open",
        "https://download.java.net/java/early_access/jdk15/29/GPL/openjdk-15-ea+29_windows-x64_bin.zip",
        Windows,
        Some(OpenJDK)
      )
    ).validate()
      .insert()
    Seq(Linux64, MacOSX, Windows).foreach(
      removeVersion("java", "15.ea.28-open", _)
    )
  }

  @ChangeSet(
    order = "053",
    id = "053-add_openjdk_java_16-ea-3",
    author = "eddumelendez"
  )
  def migrate053(implicit db: MongoDatabase): Unit = {
    List(
      Version(
        "java",
        "16.ea.3-open",
        "https://download.java.net/java/early_access/jdk16/3/GPL/openjdk-16-ea+3_linux-x64_bin.tar.gz",
        Linux64,
        Some(OpenJDK)
      ),
      Version(
        "java",
        "16.ea.3-open",
        "https://download.java.net/java/early_access/jdk16/3/GPL/openjdk-16-ea+3_osx-x64_bin.tar.gz",
        MacOSX,
        Some(OpenJDK)
      ),
      Version(
      ...
      .insert()
    Seq(Linux64, MacOSX, Windows).foreach(
      removeVersion("java", "16.ea.2-open", _)
    )
  }
